...
export const runtime = 'nodejs' // needs crypto module for IP hashing
...
export async function POST(req: NextRequest) {
  try {
    ...
    const { name, emoji, eventDate, timezone } = parsed.data
    ...
    const [{ value: activeCount }] = await db
      ...
      .where(
        and(
          eq(schema.countdowns.ipHash, ipHash),
    ...
    // ── Convert local event date to UTC ────────────────────────────────────
    // The client sends a local ISO string (e.g. "2025-12-31T23:59").
    // We treat it as the user's chosen timezone and convert to UTC for storage.
    // date-fns-tz is used for reliable timezone math.
    ...
    if (!slug) {
      return NextResponse.json(
        { error: 'Failed to generate a unique link. Please try again.' },
        { status: 500 }
      )
    }
